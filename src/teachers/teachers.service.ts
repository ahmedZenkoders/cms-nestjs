...
export class TeachersService {
    ...
    async updateTeacherProfile(email: string, updateTeacherDto: UpdateTeacherDto) {
        const teacher = await this.teacherRepository.findOne({
            where: { email: email },
        });
        if (!teacher) {
            throw new NotFoundException(
                `Teacher with email ${email} not found`,
            );
        }
    ...
    async updateTeacherProfilePicture(email: string, image: Express.Multer.File) {
        const teacher = await this.teacherRepository.findOne({
            where: { email: email },
        });
        if (!teacher) {
